...
export async function showProductCardFull(product, { isDesktop = false } = {}) {
  const { name, description, cost, image } = product;

  /* ---------- overlay ---------- */
  const overlay = document.createElement("div");
  overlay.className = `
  fixed inset-0 bg-black/40 z-50 flex
  ${isDesktop ? "items-center justify-center p-6" : "items-end"}
`;

  /* ---------- sheet ---------- */
  const sheet = document.createElement("div");
  sheet.className = isDesktop
    ? `
    w-full max-w-5xl
    max-h-[90vh]
...
${
  !isDesktop
    ...
    : `
      ...
      ">
        <h2 class="text-xl font-semibold truncate pr-4">
          ${name}
        </h2>
  ...
  bottom.addEventListener("click", async (e) => {
    ...
    try {
      ...
      // 🔥 сообщаем всем карточкам об обновлении
      ...
      res.quantity === 0 ? renderAddButton() : renderQuantity(res.quantity);
    } catch (e) {
