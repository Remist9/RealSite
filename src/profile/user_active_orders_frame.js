import { getUserActiveOrders } from "./profile_api.js";

export function userActiveOrdersFrame({ container } = {}) {
...
`;
  ...
  const content = sheet.querySelector("#active-orders-content");
  ...
  async function loadOrders() {
    try {
      const data = await getUserActiveOrders();

      console.log("Активные заказы:", data);
      ...
      <div class="w-full border rounded-2xl p-4 text-left shadow-sm bg-white">
        ...
        <div class="text-sm text-gray-600 flex flex-col gap-1">
          <div><strong>Дата создания:</strong> ${createdDate}</div>
